...
KLIK_I2C_Motor mot_L(0x08);                     //   Объявляем объект mot для работы с функциями и методами библиотеки KLIK_I2C_Motor, указывая адрес модуля на шине I2C.
                                                //   Если объявить объект без указания адреса (KLIK_I2C_Motor mot;), то адрес будет найден автоматически.
KLIK_I2C_Motor mot_R(0x0A);                                                

float R = 24.5;   // расстояние от центра между ведущими колёсами к третьему колесу в мм. Часто это длина робота.
float r = 24.5;  // радиус колеса
float L = 0.0; // переменная Длина дуги поворота
float k=8;
float angle =15.0; //угол поворота
...
void loop(){                                      //
    mot_L.setSpeed(-36, MOT_RPM, L, MOT_MET);     //   Запускаем мотор на скорости 100 об/мин с остановкой мотора через L метра.
    //mot_R.setSpeed(-36, MOT_RPM, L, MOT_MET);
