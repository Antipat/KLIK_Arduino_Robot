#include "NikiR_I2C_Motor.h"																									//
																																//
//		Инициализация модуля:																									//	Возвращаемое значение: результат инициализации.
bool	NikiR_I2C_Motor::_begin			(void){																				//	Параметр: отсутствует
...
bool	NikiR_I2C_Motor::reset			(void){																				//	Параметр:				отсутствует.
...
bool	NikiR_I2C_Motor::changeAddress	(uint8_t newAddr){																	//	Параметр:				newAddr - новый адрес модуля (0x07 < адрес < 0x7F).
			if(valAddr){																										//	Если модуль был инициализирован, то ...
...
bool	NikiR_I2C_Motor::getPullI2C		(void){																				//	Параметр:				отсутствует.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
...
bool	NikiR_I2C_Motor::setPullI2C		(bool f){																			//	Параметр:				флаг подтяжки линий шины I2C.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
			//	Считываем первые два регистра: регистр флагов и регистр битов:													//
				if(_readBytes(REG_FLAGS_0,2)==false ){ return false; }															//	Читаем 2 байта начиная с регистра «REG_FLAGS_0» в массив «data».
			//	Проверяем поддерживает ли модуль управление подтяжкой линий шины I2C:											//
...
bool	NikiR_I2C_Motor::setFreqPWM		(uint16_t frequency){																//	Параметр:				частота в Гц (25-1000).
			if(valAddr){																										//	Если модуль был инициализирован, то ...
...
bool	NikiR_I2C_Motor::setMagnet		(uint8_t sum){																		//	Параметр:				sum - количество магнитов от 1 до 63.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
...
uint8_t	NikiR_I2C_Motor::getMagnet		(void){																				//	Параметр:				отсутствует.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
...
bool	NikiR_I2C_Motor::setReducer		(float gear){																		//	Параметр:				gear - передаточное отношение редуктора от 0.01 до 167'772.15.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
...
float	NikiR_I2C_Motor::getReducer		(void){																				//	Параметр:				отсутствует.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
...
bool	NikiR_I2C_Motor::setError		(uint8_t deviation){																//	Параметр:				deviation - процент максимально допустимого отклонения реальной скорости от заданной, превышение которого приведёт к установке флага ошибки
			if(valAddr){																										//	Если модуль был инициализирован, то ...
			//	Проверяем полученные данные:																					//
...
uint8_t	NikiR_I2C_Motor::getError		(void){																				//	Параметр:				отсутствует.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
...
bool	NikiR_I2C_Motor::setSpeed		(float valSpeed, uint8_t typeSpeed, float valStop, uint8_t typeStop){				//	Параметры:				valSpeed - значение скорости, typeSpeed - тип значения скорости MOT_RPM / MOT_PWM / MOT_M_S, valStop - значение условия остановки, typeStop - тип условия остановки: MOT_MET / MOT_REV / MOT_SEC.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
				uint8_t reg;																									//	
...
float	NikiR_I2C_Motor::getSpeed		(uint8_t type){																		//	Параметр:				type - тип возвращаемого значения: MOT_RPM / MOT_PWM / MOT_M_S
			float valSpeed=0.0f;																								//
			if(valAddr){																										//	Если модуль был инициализирован, то ...
				uint8_t reg;																									//	
...
bool	NikiR_I2C_Motor::setStop			(float value, uint8_t type){														//	Параметры:				value - значение условия остановки, type - тип условия остановки: MOT_MET / MOT_REV / MOT_SEC.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
...
float	NikiR_I2C_Motor::getStop			(uint8_t type){																		//	Параметр:				type - тип запрашиваемого значения условия: MOT_MET / MOT_REV / MOT_SEC.
...
bool	NikiR_I2C_Motor::setStopNeutral	(bool f){																			//	Параметр:				f - флаг true/false.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
...
bool	NikiR_I2C_Motor::getStopNeutral	(void){																				//	Параметр:				отсутствует.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
			//	Считываем состояние регистра флагов:																			//
				if(_readBytes(REG_MOT_FLG,1)==false){return 0;}																	//	Читаем 1 байт регистра «REG_MOT_FLG» в массив «data».
			//	Возвращаем результат:																							//
...
bool	NikiR_I2C_Motor::setDirection	(bool flgCKW){																		//	Параметры:				flgCKW - флаг вращения вала по ч.с. при положительной скорости.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
			//	Готовим данные:																									//
...
bool	NikiR_I2C_Motor::getDirection	(void){																				//	Параметр:				отсутствует.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
...
bool	NikiR_I2C_Motor::setInvGear		(bool invRDR, bool invPIN){															//	Параметры:				invRDR - флаг инверсии вращения редуктора, invPIN - флаг инверсии полярности мотора.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
			//	Готовим данные:																									//
				if(_readBytes(REG_BITS_2,1)==false){return 0;}																	//	Читаем 1 байт регистра «REG_BITS_2» в массив «data».
...
uint8_t	NikiR_I2C_Motor::getInvGear	(void){																					//	Параметр:				отсутствует.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
			//	Считываем значение регистра битов:																				//
				if(_readBytes(REG_BITS_2,1)==false){return 0;}																	//	Читаем 1 байт регистра «REG_BITS_2» в массив «data».
			//	Возвращаем результат:																							//
...
float	NikiR_I2C_Motor::getSum			(uint8_t type){																		//	Параметр:				type - тип запрашиваемого значения: MOT_MET / MOT_REV.
...
bool	NikiR_I2C_Motor::setVoltage		(float motVoltage){																	//	Параметр:				motVoltage - напряжение в В.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
			//	Готовим данные:																									//
...
float	NikiR_I2C_Motor::getVoltage		(void){																				//	Параметр:				отсутствует.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
...
bool	NikiR_I2C_Motor::setNominalRPM	(uint16_t value){																	//	Параметр:				value - скорость вращения вала редуктора от 0 до 65'535 об/мин.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
			//	Готовим данные:																									//
...
uint16_t NikiR_I2C_Motor::getNominalRPM	(void){																				//	Параметр:				отсутствует.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
...
bool	NikiR_I2C_Motor::saveManufacturer(uint64_t code){																	//	Параметр:				code - код разработчика.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
			//	Ждём:																											//
...
bool	NikiR_I2C_Motor::_readBytes		(uint8_t reg, uint8_t sum){															//	Параметры:				reg - номер первого регистра, sum - количество читаемых байт.
...
bool	NikiR_I2C_Motor::_writeBytes	(uint8_t reg, uint8_t sum, uint8_t num){												//	Параметры:				reg - номер первого регистра, sum - количество записываемых байт, num - номер первого элемента массива data.
